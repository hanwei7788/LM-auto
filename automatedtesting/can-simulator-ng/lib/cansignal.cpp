 ...
 */
uint64_t CANSignal::getRawValue()
{
    uint64_t value = llrint((m_value.toDouble() - offset) / factor);
    return value;
}
...
bool CANSignal::setValueFromRaw(uint64_t rawValue)
{
    ...
    if (m_value.type() == Value::Double) {
        ...
        unsigned long value = (rawValue * factor) + offset;
        val = Value(value);
    } else {
...
bool CANSignal::parseValue(const std::string &valueString, Value &value)
{
    ...
    if (!m_valueType.compare("double")) {
        ...
        value = Value(static_cast<unsigned long>(llrint(parsedValue)));
    } else {
        value = Value(static_cast<int>(llrint(parsedValue)));
    }
...
std::string CANSignal::toString(bool details) const
{
    ...
    if (!m_valueType.compare("double")) {
        ...
        sstream << ", value: " << m_value.toUnsigned() << ", range [" << llrint(minimum) << ", " << llrint(maximum) << "] " << unit << "\n";
    } else {
        sstream << ", value: " << m_value.toInt() << ", range [" << llrint(minimum) << ", " << llrint(maximum) << "] " << unit << "\n";
    }
