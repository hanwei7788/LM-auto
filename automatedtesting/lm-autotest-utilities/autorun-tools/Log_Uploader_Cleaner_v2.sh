...
# Compress all results if a test log is found
cd "${autotest_root}/c4c-functional-tests"
current_results="TestReports_$1/$2"
report_archive="${autotest_root}/lm-autotest-utilities/autorun-tools/uploader/$1-$2.tar.gz"
# Test for file
testresultfile="${autotest_root}/c4c-functional-tests/TestReports_$1/$2/$1-$2-log_TestRun.txt"
if [ -f "${testresultfile}" ]
then
  # Compress
  tar -zcvf "${report_archive}" "$current_results"
  # Upload archives
